...
func BenchmarkPublish(b *testing.B) {
	configSync, err := configuration.LoadConfig("../config.json")
	if err != nil {
		b.Error(err)
		return
	}
	configSync.Debug = false
	configSync.FatalKafkaError = false
	configSync.Validate = true
	configSync.ValidateAllowUnknownField = true
	configSync.ValidateAllowMissingField = true
	configSync.Log = "stdout"
	configSync.SyncKafka = true
	configSync.SyncKafkaIdempotent = false

	b.Run("sync", benchmarkPublish(configSync))

	var configSyncIdempotent configuration.ConfigStruct = *configSync
	configSyncIdempotent.SyncKafkaIdempotent = true
	b.Run("sync idempotent", benchmarkPublish(&configSyncIdempotent))

	var configAsync configuration.ConfigStruct = *configSync
	configAsync.SyncKafkaIdempotent = false
	configAsync.SyncKafka = false
	b.Run("async", benchmarkPublish(&configAsync))
}

func benchmarkPublish(originalConfig configuration.Config) func(b *testing.B) {
	return func(b *testing.B) {
		ctx, cancel := context.WithCancel(context.Background())
		defer time.Sleep(10 * time.Second) //wait for container shutdown
		defer cancel()

		config, err := server.New(ctx, originalConfig)
		if err != nil {
			b.Error(err)
			return
		}
