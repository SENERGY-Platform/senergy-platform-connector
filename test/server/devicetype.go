...
func CreateDeviceType(conf lib.Config, managerUrl string) (typeId string, serviceTopic string, err error) {
	...
	time.Sleep(10 * time.Second)
	return devicetype.Id, model.ServiceIdToTopic(devicetype.Services[0].Id), nil
}
